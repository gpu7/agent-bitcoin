...
def test_create_invoice_quote_includes_fee(clear_payment_env, monkeypatch):
    monkeypatch.setenv("FEE_AMOUNT_SATS", "1000")
...
def test_validate_quote_rejects_bolt11_mismatch(clear_payment_env, monkeypatch):
    ...
    with patch("agent_bitcoin.client.LNDClient") as mock_cls:
        ...
        quote = InvoiceQuote(
            ...
            amount_sats=2000,
            platform_fee_sats=1000,
            transaction_fee_sats=1000,
            total_cost_sats=3000,
        )
...
def test_build_payer_decision_inputs_ok(clear_payment_env, monkeypatch):
    ...
    with patch("agent_bitcoin.client.LNDClient") as mock_cls:
        ...
        assert inputs.platform_fee_sats == 1000
        assert inputs.transaction_fee_sats == 1000
        assert inputs.total_cost_sats == 3000
