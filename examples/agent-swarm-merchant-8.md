# 8-agent merchant swarm

Eight Nostr identities (`a1`…`a8`), signed relay events, **one** winner pays `GET /paid/finance/mempool-feerate` (100 sats). Same jobs as the [two-agent demo](./agent-swarm-merchant-2.md). Coded policy picks the payer. Optional Grok explains in one sentence each and **never** pays. **`--no-llm` is the required path.**
...
Not Grok and not Lightning. `--no-llm` only skips an optional one-sentence explanation. Coded policy picks the payer. Eight scores, one winner.
...
The eight-agent demo stays **`--resolve hash`**. Puzzle (`fee-sats`) and **llm-gate** are two-agent only — [agent-swarm-merchant-2.md](./agent-swarm-merchant-2.md).
...
Sequence of a paid GET: [docs/architecture.md — L402 request sequence](../docs/architecture.md#l402-request-sequence). Operator gateway: [docs/l402-aperture.md](../docs/l402-aperture.md). Two-agent: [agent-swarm-merchant-2.md](./agent-swarm-merchant-2.md).
