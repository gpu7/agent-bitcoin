...
AWS  agent-l402-aperture :8081
        ├─ LND gRPC  agent-payment-decision-lnd:10009  (invoice.macaroon)
...
- AWS + Mac regtest LND up and **unlocked**
- Channel Mac → AWS with **≥ 1,000 local sats** on the Mac
- Docker network `regtest_regtest` and volume `agent-bitcoin_lnd-data`
...
This does **not** change `startup-aws.sh` / Loop.
...
Expect `status=200 paid=True` and JSON `{"ok": true, "service": "l402-demo", "network": "regtest", "msg": "hello"}`.
...
| `l402/aperture.regtest.yaml` | `price: 1000`, `insecure: true` |
...
Signet and mainnet get their own compose overlays and PRs. Same `:8081` + SG `/32`. Mainnet pay still needs a written go. Invoice-only macaroon is required on mainnet (Aperture already requests `invoice.macaroon` from `macdir`).
