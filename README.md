...
Lightning invoice/pay does **not** require Nostr. L402 **finance** tools do not use it. Paid local checks: `POST /paid/nostr/event-verify` (NIP-01 id+sig) and `POST /paid/nostr/npub-decode` (NIP-19 bech32 → hex; `nsec` rejected) at 100 sats — [docs/l402-aperture.md](docs/l402-aperture.md). Nostr is an optional layer so agents can have a public identity and, if you opt in, a limited wallet connection.
